function [searchlight_test_class, searchlight_accuracies] = ...
         searchlight_svm(activation_matrix, ...
...
%                          ground_truth)
% 
% Linear support vector machine classifier analysis on activation data
...
training_class = [];
for i = 1:size(training_times, 2)
    training_class = [training_class; ...
                     i*ones(size(training_times{i}))];
end
...
try 
    ...
    svm_mod = svmtrain2(training_class, sl_training_matrix, '-q');

    % Test classifier on test trial patterns in this searchlight
    test_class = svmpredict2(ground_truth, sl_testing_matrix, svm_mod, '-q');
