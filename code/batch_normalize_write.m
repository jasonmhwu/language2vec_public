function batch_normalize_write(subj_info_struct, base_dir)
...
for subj_num = 1:length(subj_info_struct) 
    ...
    load('lin_new_normalize_template.mat');
    ...
    for this_bold_dir_idx = 1:length(subj_bold_dirs),
        ...
        [these_bold_files,dirs]=spm_select('ExtFPList',this_bold_dir_string, ...
                                           ['^ar' this_subj_info.ScanningDir '.*\.nii$']);
        ...
        normalized_bold_files_exist = cellfun(@(path, fn) ...
            logical(exist(fullfile(path, ['war' fn '.nii']))), paths, fns);
    ...
    %%% SPM select forward .mat file
    [forward,dirs]=spm_select('List', T1_dir, ...
                                       ['^' this_subj_info.ScanningDir '.*_seg_sn\.mat$']);
    matlabbatch{1}.spm.spatial.normalise.write.subj.matname = cell(1);
    matlabbatch{1}.spm.spatial.normalise.write.subj.matname{1} = fullfile( T1_dir, forward);
