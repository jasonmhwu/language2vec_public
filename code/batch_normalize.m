function batch_normalize(subj_info_struct)
...
for subj_num = 1:length(subj_info_struct) 
    ...
    fprintf('Running batch preprocessing on subject %d, %s\n', ...
        this_subj_info.SubjectNum, this_subj_info.PrePost);
    %%% Load in the manually saved normalize .mat file from subj 1's data
    load('lin_normalize_template.mat');
    %%% Automagically find the template to realign to (depends on where spm
    %%% is installed on system)
    spmdir = fileparts(which('spm'));
    template_location = fullfile(spmdir, 'templates', 'EPI.nii');
    ...
    for this_bold_dir_idx = 1:length(subj_bold_dirs),
        ...
        normalized_bold_files_exist = cellfun(@(path, fn) ...
            logical(exist(fullfile(path, ['w' fn '.nii']))), paths, fns);
        ...
        %%% Put this subjectfcf's BOLD files (not already aligned) into the jobs struct
    ...
    %%% Pull out mean realigned image from first bold dir
    first_bold_dir_string = fullfile(base_dir, this_subj_base_dir, subj_bold_dirs{1});
    [mean_image_file,dirs]=spm_select('ExtFPList',first_bold_dir_string, ...
                                 ['^mean' this_subj_info.ScanningDir '.*\.nii$']);
    matlabbatch{1}.spm.spatial.normalise.estwrite.subj.source = {mean_image_file};
